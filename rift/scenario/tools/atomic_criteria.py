...
from rift.scenario.tools.traffic_events import TrafficEvent, TrafficEventType
...
class MaxVelocityTest(Criterion):
    ...
    def update(self):
        ...
        velocity = convert_velocity(CarlaDataProvider.get_velocity(self.actor))
...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        if convert_velocity(CarlaDataProvider.get_velocity(self.actor)) < self.EPSILON:
            return
...
class ActorBlockedTest(Criterion):
    ...
    def update(self):
        ...
        if self._active:
            linear_speed = convert_velocity(CarlaDataProvider.get_velocity(self.actor))
...
class RunningStopTest(Criterion):
    ...
    def update(self):
        ...
        if not self._stop_completed:
            current_speed = convert_velocity(CarlaDataProvider.get_velocity(self.actor))
...
class MinimumSpeedRouteTest(Criterion):
    ...
    def update(self):
        ...
        background_vehicles = CarlaDataProvider.get_ego_nearby_agents(self.actor.id)
        ...
        if background_vehicles:
            ...
            for vehicle in background_vehicles:
                frame_mean_speed += convert_velocity(CarlaDataProvider.get_velocity(vehicle))
