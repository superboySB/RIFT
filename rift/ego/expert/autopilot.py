...
from rift.ego.expert.nav_planner import PIDController, interpolate_trajectory
...
class AutoPilot(object):
    def __init__(self, config=None, logger=None):
        ...
        self.logger = logger
        self.step = -1
        self.target_speed = CarlaDataProvider.get_desired_speed()

        self.save_path = None

        self.render_bev = False

        # self.gps_buffer = deque(maxlen=1) # Stores the last x updated gps signals.

        # Dynamics models
        self.frame_rate = config['frame_rate']
        self.frame_rate_sim = config['frame_rate']
        ...
        self.target_speed_slow = self.target_speed - 3   # Speed at junctions, m/s
    ...
    def tick_autopilot(self, input_data=None):
        if input_data is not None:
            ...
            ego_trans = CarlaDataProvider.get_transform(self._vehicle)
            ego_loc = ego_trans.location
            gps = np.array([ego_loc.x, ego_loc.y])
            speed = CarlaDataProvider.get_velocity(self._vehicle).length()  # m/s
            compass = np.deg2rad(ego_trans.rotation.yaw)
    ...
    def _get_control(self, input_data=None, viz_route=None, steer=None, throttle=None,
        ...
        if viz_route:
            ...
            draw_route(self._world, vehicle=self._vehicle, waypoint_route=waypoint_route_draw, frame_rate=self.frame_rate)
    ...
    def save(self, target_point, next_target_point, steer, throttle, brake, target_speed, tick_data):
        ...
        data = {
                ...
                'x_command': target_point[0], # in global coordiantes
                'y_command': target_point[1], #
        ...
        if vehicle_hazard is None or walker_hazard is None:
            ...
            for vehicle in vehicles:
                ...
                if (vehicle.get_location().distance(vehicle_location) < detection_radius):
                    ...
                    nearby_agents[vehicle.id] = veh_future_bbs
            ...
            for i in range(number_of_future_frames):
                ...
                if (lane_change == False):
                    ...
                    i_stuck = i
                    for id, traffic_participant in nearby_agents.items():
                        if self.render_bev==False and self.junction==False and i > number_of_future_frames_no_junction:
                                break
            ...
            for _, traffic_participant in nearby_agents.items():
            ...
            self.future_states = {'walker': nearby_walkers, 'vehicle': nearby_agents}
    ...
    def _intersection_check(self, ego_wps):
        ...
        # Intersection checks with ego vehicle
        # -----------------------------------------------------------
        number_of_interpolation_frames = self.frame_rate // 2 # only checked for 20fps
        ...
        for i in range(1, 1+ego_wps.shape[1]): # check dimension!!!
            if self.render_bev==False and self.junction==False and i > number_of_future_frames_no_junction:
                alpha = 255
                color_value = 50
                break
