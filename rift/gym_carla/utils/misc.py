...
def get_lane_dis(local_route_waypoints, x, y):
    """
        Calculate distance from (x, y) to waypoints.
        :param local_route_waypoints: a list of list storing waypoints like [waypoint0, waypoint1, ...]
    ...
    eps = 1e-5
    dis_min = 99999
    waypt = local_route_waypoints[0]
    for pt in local_route_waypoints:
        d = np.sqrt((x-pt.transform.location.x)**2 + (y-pt.transform.location.y)**2)
        if d < dis_min:
            dis_min = d
            waypt = pt
    vec = np.array([x - waypt.transform.location.x, y - waypt.transform.location.y])
    lv = np.linalg.norm(np.array(vec)) + eps
    w = np.array([np.cos(waypt.transform.location.x/180*np.pi), np.sin(waypt.transform.location.y/180*np.pi)])
