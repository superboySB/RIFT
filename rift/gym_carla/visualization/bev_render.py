...
class BirdeyeRender(object):
        ...
        for i in range(max(0, lp - num), lp):
            for ID, poly in actor_polygons[i].items():
                ...
                if ID in all_ego_ids:
                    color = pygame.Color(255, 0, 0)  # Ego -> red
                elif ID in all_CBV_ids:
                    color = pygame.Color(math.floor(0.5 * 255), 0, math.floor(0.5 * 255))  # CBV -> purple
                else:
                    if actor_type == 'vehicle':
                        color = pygame.Color(0, 238, 255)  # BV -> sky blue
                    elif actor_type == 'walker':
                        color = pygame.Color(255, 255, math.floor(color_value * 255))  # yellow
                pygame.draw.polygon(surface, color, corners)

    def render_waypoints(self, surface, local_route_waypoints, world_to_pixel):
        if self.red_light:
            color = pygame.Color(math.floor(0.5 * 255), 0, math.floor(0.5 * 255))  # purple
        else:
            color = pygame.Color(254, 129, 125)  # orange
        corners = []
        for waypoint in local_route_waypoints:
            corners.append(waypoint.transform.location)
        corners = [world_to_pixel(p) for p in corners]
        route_width = 8
    ...
    def render(self, render_types=None):
        ...
        self.render_waypoints(self.waypoints_surface, self.local_route_waypoints, self.map_image.world_to_pixel)
