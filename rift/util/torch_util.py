...
def set_torch_variable():
    global DEVICE
    cuda_devices = os.getenv("CUDA_VISIBLE_DEVICES")
    if cuda_devices is not None:
        device_list = list(map(int, cuda_devices.split(',')))
        if len(device_list) == 1:
            cuda_device_id = device_list[0]
            print(f">> Using GPU device {cuda_device_id}")
        else:
            raise ValueError("Currenlty, only one GPU is supported.")
    else:
        raise ValueError("CUDA_VISIBLE_DEVICES is not set. Please set it to a valid device ID.")
    ...
    carla_device_id = 0 if cuda_device_id == 0 else cuda_device_id + 1
    # In some machines, for some unknown reasons, -graphicsadapter=1 is not available. 
    # For example, with 4 GPUS, it might be: GPU0 -graphicsadapter=0, 
    # GPU1 -graphicsadapter=2, GPU2 -graphicsadapter=3, GPU3 -graphicsadapter=4.

    return carla_device_id
