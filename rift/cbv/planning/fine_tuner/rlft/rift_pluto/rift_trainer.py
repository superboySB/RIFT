...
class LightningTrainer(L.LightningModule):
    ...
    def _compute_objectives(self, cur_res, cur_data, batch) -> Dict[str, torch.Tensor]:
        ...
        # rift loss
        rift_loss = self.get_rift_loss(probability, cur_data, batch)
        ...
        return {
            "loss": loss,
            "rift_loss": rift_loss.item(),
            # "drivable_area_loss": drivable_area_loss.item(),
        }

    def get_rift_loss(self, probability, cur_data, batch):
        '''
        Compute the rift loss.
        '''
        ...
        unclipped_objective = group_advantage * ratio
        clipped_objective = group_advantage * torch.clamp(ratio, 0.8, 1.2)
        min_objective = torch.min(unclipped_objective, clipped_objective)
        max_objective = torch.max(min_objective, group_advantage * 3.0)
        objective = torch.where(group_advantage < 0, max_objective, min_objective)
