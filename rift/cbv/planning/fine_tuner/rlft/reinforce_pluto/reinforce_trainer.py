...
class LightningTrainer(L.LightningModule):
    ...
    def _compute_objectives(self, cur_res, cur_data, batch) -> Dict[str, torch.Tensor]:
        ...
        # reinforce loss
        reinforce_loss = self.get_reinforce_loss(probability, best_r_idx, best_m_idx, batch)
    ...
    def get_reinforce_loss(self, probability, best_r_idx, best_m_idx, batch):
        '''
        ...
        chosen_log_probs = log_action_probs[torch.arange(bs), best_r_idx, best_m_idx]  # (bs,)

        # return_torch
        return_torch = batch['return_torch']  # (bs,)

        # reinforce loss
        reinforce_loss = -torch.mean(chosen_log_probs * return_torch.detach())        
