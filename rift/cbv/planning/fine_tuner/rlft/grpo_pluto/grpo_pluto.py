...
class GRPOPluto(RLFTPluto):
    ...
    def __init__(self, config, logger):
        ...
        # the reference model
        self.ref_model = CUDA(PlanningModel(radius=self.radius))
        self.ref_model.load_state_dict(self.load_infer_checkpoint(self._ckpt_path, device_name=self.device))
        self.ref_model.eval()
    ...
    def get_action(self, CBVs_obs_list, infos, deterministic=False) -> Dict[str, List[Dict[Any, Any]]]:
        ...
        CBVs_ref_group_logits = [{} for _ in range(self.num_scenario)]
        ...
        for info, CBVs_obs in zip(infos, CBVs_obs_list):
            ...
            with torch.no_grad():
                ...
                ref_pluto_output = self.ref_model(pluto_feature_data)
            for index, (CBV_id, CBV_obs) in enumerate(CBVs_obs.items()):
                ...
                (
                    ...
                    CBVs_ref_group_logits[env_id][CBV_id],
                    CBVs_group_advantage[env_id][CBV_id]
                ) = self._get_action(pluto_feature_data, pluto_output, ref_pluto_output, CBV_obs, env_id, CBV_id, index)
        ...
        data = {
            ...
            'CBVs_actions_ref_group_logits': CBVs_ref_group_logits,
            'CBVs_group_advantage': CBVs_group_advantage
        }

        return data

    def _get_action(self, pluto_feature_data, pluto_output, ref_pluto_output, CBV_obs, env_id, CBV_id, index):
        ...
        # grpo require the group advantage
        if self.mode == 'train':
            ...
            ref_probability = ref_pluto_output["probability"][index].cpu().numpy()  # [padded_R, M]
            valid_ref_probability = ref_probability[r_valid_mask.cpu().numpy()]

            ref_group_logits = {
                'logits': valid_ref_probability,  # [Valid_R, M]
                'valid_mask': np.ones_like(valid_ref_probability, dtype=np.bool_)  # [Valid_R, M]
            }
        else:
            group_advantage = None
            old_group_logits = None
            ref_group_logits = None
        ...
        return (throttle, steer, brake), old_group_logits, ref_group_logits, group_advantage